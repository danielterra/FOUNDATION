...
<div class="node-wrap">
  <div class="node">
    <div class="side-tab">
      <span class="material-symbols-outlined icon">http</span>
    </div>
    <div class="content">
...
<style>
  ...
  .node {
    ...
    width: 220px;
    cursor: pointer;
    background: var(--automation-node-request-bg);
    color: var(--automation-node-request-text);
    overflow: hidden;
  }
  .side-tab {
    width: 28px;
    flex-shrink: 0;
    background: var(--automation-node-request-tab);
    display: flex;
    align-items: center;
    justify-content: center;
  }
  .icon { font-size: 16px; color: var(--automation-node-request-bg); font-variation-settings: 'FILL' 1; }
